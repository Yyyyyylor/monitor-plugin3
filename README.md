...
### 并发监控与调度间隔修复（2026-08-02，v3.3.1）

**现象**：日志显示同一账号 0.2 秒内被爬取两次、多轮"本轮监控完成"、消息投递循环 1 秒内触发多次；且监控循环仅启动时执行一次后长时间无后续。

**根因**：
1. **多实例并发**：AstrBot 热重载/重复加载产生多个插件实例时，新旧实例的监控循环并发执行。由于新旧实例是不同模块对象（各自独立 `_monitor_lock`），模块级锁无法串行化
2. **间隔不更新**：统一/分层/维护三个循环的 `interval` 只在启动时读取一次，WebUI 修改配置不生效

**修复**：
1. **跨模块实例共享的控制器注册表**（经 `sys.modules` 命名空间）：新控制器启动时自动停止仍在运行的旧控制器 → 同一时刻仅一个监控控制器
2. 三个后台循环**每轮动态读取间隔** + 输出下次执行时间日志
3. 无变化通知（`non_change_message`）默认开启，且 `/stopmessage off` 暂停投递时不再误发

**验证**：`tests/test_monitor_controller.py` 3 项回归（单实例注册 / 新停旧 / stop 幂等）+ 模拟运行验证 interval 60→5 分钟动态生效。
