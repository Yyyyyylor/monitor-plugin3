"""SQLAlchemy ORM 模型 — 4 张业务表（plan-v3 §5.5 / §12.3）。

- MonitoredUser：监控账号（含软删除/回收站、频率、QQ 会话绑定 bound_umo）
- CurrentInventoryState：当前库存基准快照（zlib 压缩，每用户一条）
- InventoryChange：变化事件
- SnapshotArchive：历史快照归档
login_rate_limit（Web 登录限速专用）已剔除。
"""
...
class MonitoredUser(Base):
    ...
    bound_umo: Mapped[str | None] = mapped_column(String(512), default=None)  # unified_msg_origin
