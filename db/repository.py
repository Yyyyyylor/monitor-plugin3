...
从 monitor/src/db/repository.py 迁移（plan-v3 §12.1），适配插件环境：
- 相对导入
- MonitoredUser 增加 bound_umo 绑定
- 新增 get_users_by_umo / get_archives_by_date
- 剔除 login_rate_limit 相关（Web 登录专用）
其余（LRU 快照缓存、原子事务、软删除/回收站、导入导出）原样保留。
"""
...
from ..core.models.item import ChangeEvent, ChangeType, InventorySnapshot, Item
from .database import async_session_factory
from .models import CurrentInventoryState, InventoryChange, MonitoredUser, SnapshotArchive
...
def _iso_utc(dt: datetime | date | None) -> str | None:
    """将 datetime/date 转为 ISO 字符串，UTC datetime 带 Z 后缀。"""
    if dt is None:
        return None
...
async def get_user_by_steam_id(steam_id: str) -> MonitoredUser | None:
    """根据 Steam ID 查询用户（含回收站）。"""
    async with async_session_factory() as session:
        result = await session.execute(
...
async def get_users_by_umo(umo: str) -> list[MonitoredUser]:
    """获取绑定到同一会话标识的所有账号（支持一个 QQ 监控多个 Steam）。"""
    async with async_session_factory() as session:
        result = await session.execute(
            select(MonitoredUser).where(
                MonitoredUser.bound_umo == umo,
                MonitoredUser.is_active.is_(True),
                MonitoredUser.deleted_at.is_(None),
            )
        )
...
async def permanent_delete_user(steam_id: str) -> bool:
    """永久删除用户及其所有数据（不可恢复）。"""
    async with async_session_factory() as session:
        await session.execute(delete(MonitoredUser).where(MonitoredUser.steam_id == steam_id))
        await session.execute(delete(CurrentInventoryState).where(CurrentInventoryState.steam_id == steam_id))
        await session.execute(delete(InventoryChange).where(InventoryChange.steam_id == steam_id))
        await session.execute(delete(SnapshotArchive).where(SnapshotArchive.steam_id == steam_id))
        await session.commit()
...
async def upsert_user(
    steam_id: str, nickname: str | None = None, bound_umo: str | None = None
) -> MonitoredUser:
    """添加或更新用户。如果用户在回收站中，自动恢复。

    bound_umo 仅在非空时更新（后绑者生效，plan-v3 R9）。
    """
    async with async_session_factory() as session:
        result = await session.execute(
            select(MonitoredUser).where(MonitoredUser.steam_id == steam_id)
        ...
        if user:
            ...
            if bound_umo:
                user.bound_umo = bound_umo
            if user.deleted_at is not None:
                user.deleted_at = None  # 从回收站恢复
                user.is_active = True
        else:
            user = MonitoredUser(steam_id=steam_id, nickname=nickname, bound_umo=bound_umo)
            session.add(user)
        await session.commit()
        return user


async def set_user_nickname(steam_id: str, nickname: str) -> bool:
    """修改账号昵称。"""
    async with async_session_factory() as session:
        result = await session.execute(
            select(MonitoredUser).where(MonitoredUser.steam_id == steam_id)
...
async def cleanup_old_changes(days: int | None = None) -> int:
    """清理超过保留天数的变化事件，返回删除行数。"""
    from ..core.config import settings
    retention = days if days is not None else settings.change_retention_days
...
async def cleanup_old_archives(days: int | None = None) -> int:
    """清理超过保留天数的归档快照。"""
    from ..core.config import settings
    retention = days if days is not None else settings.archive_retention_days
    cutoff = datetime.now(timezone.utc) - timedelta(days=retention)
    async with async_session_factory() as session:
        result = await session.execute(
...
async def get_archives_by_date(steam_id: str, target: date) -> SnapshotArchive | None:
    """获取指定日期的归档快照，不存在返回 None。"""
    ...
    async with async_session_factory() as session:
        result = await session.execute(
            ...
            .where(
                SnapshotArchive.steam_id == steam_id,
                SnapshotArchive.captured_at >= day_start,
                SnapshotArchive.captured_at < day_end,
            )
            .order_by(SnapshotArchive.captured_at.asc())
        )
        return result.scalars().first()


async def load_archive_items(archive: SnapshotArchive) -> dict[str, Item]:
    """解压归档快照为物品字典。"""
    items_raw = _decompress(archive.snapshot_data)
    return {aid: Item.from_dict(data) for aid, data in items_raw.items()}
...
async def export_all_data() -> dict[str, Any]:
    ...
    for u in all_users:
        entry: dict[str, Any] = {
            ...
            "bound_umo": getattr(u, "bound_umo", None),
...
async def import_all_data(data: dict[str, Any]) -> dict[str, int]:
    ...
    for entry in users_list:
        ...
        async with async_session_factory() as session:
            ...
            # 会话绑定：仅当导出数据有值且本地为空时绑定
            if entry.get("bound_umo") and user.bound_umo is None:
                user.bound_umo = entry["bound_umo"]
            await session.commit()
